...
fn ui_gateway_message_integration() {
    ...
    let msg = converter.marshal(UiMessage::GetNodeDescriptor).expect(
        "Couldn't marshal GetNodeDescriptor message",
    );

    let descriptor_client = ClientBuilder::new(
        format!("ws://{}:{}", localhost(), DEFAULT_UI_PORT).as_str(),
    ...
    let shutdown_msg = converter.marshal(UiMessage::ShutdownMessage).expect(
        "Couldn't marshal ShutdownMessage",
    );

    let shutdown_client = ClientBuilder::new(
        format!("ws://{}:{}", localhost(), DEFAULT_UI_PORT).as_str(),
    ...
    rt.block_on(descriptor_client).expect(
        "Couldn't block on descriptor_client",
    );
    rt.block_on(shutdown_client).expect(
        "Couldn't block on shutdown_client",
    );
    rt.shutdown_on_idle().wait().expect(
        "Couldn't wait on shutdown_on_idle",
    );
