...
pub struct NodeConversation {
    ...
    message_body_receive_rx: Receiver<Option<MessageBody>>,
}

impl NodeConversation {
    pub fn new (context_id: u64, message_body_send_tx: Sender<Result<MessageBody, u64>>, message_body_receive_rx: Receiver<Option<MessageBody>>) -> Self {
        Self {
            context_id,
    ...
    pub fn transact(&self, mut outgoing_msg: MessageBody) -> Result<MessageBody, ClientError> {
        ...
        match self.message_body_send_tx.send (Ok(outgoing_msg)) {
            Ok (_) => match self.message_body_receive_rx.recv() {
                Ok(Some(body)) => Ok(body),
                Ok(None) => Err(ClientError::ConnectionDropped(String::new())),
...
mod tests {
    ...
    fn handles_successful_transaction () {
        ...
        message_body_receive_tx.send(Some (UiShutdownResponse{}.tmb(42))).unwrap();
    ...
    #[test]
    fn handles_closed_conversation () {
        let (message_body_send_tx, message_body_send_rx) = unbounded();
        let (message_body_receive_tx, message_body_receive_rx) = unbounded();
        let subject = NodeConversation::new (42, message_body_send_tx, message_body_receive_rx);
        message_body_receive_tx.send(None).unwrap();
