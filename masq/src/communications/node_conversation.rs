...
impl Drop for NodeConversation {
    fn drop(&mut self) {
        let _ = self.conversations_to_manager_tx.try_send (OutgoingMessageType::SignOff(self.context_id()));
    }
}
...
impl NodeConversation {
    ...
    pub fn send(&self, outgoing_msg: MessageBody) -> Result<(), ClientError> {
        ...
        let result = match self.conversations_to_manager_tx.send (OutgoingMessageType::FireAndForgetMessage(outgoing_msg.clone(), self.context_id())) {
            Ok(_) => match self.manager_to_conversation_rx.recv() {
                Ok(Ok(_)) => unimplemented!("Shouldn't have gotten an Ok back"),
                Ok(Err(NodeConversationTermination::Graceful)) => unimplemented!("Graceful"),
                Ok(Err(NodeConversationTermination::Resend)) => unimplemented!("Resend"),
                Ok(Err(NodeConversationTermination::Fatal)) => unimplemented!("Fatal"),
                Ok(Err(NodeConversationTermination::FiredAndForgotten)) => Ok(()),
                Err(e) => unimplemented!("ConnectionDropped: {:?}", e),
            },
            Err(_) => unimplemented!("ConnectionDropped"),
        };
        result
    }
...
eprintln! ("recv_result: {:?}", recv_result);
                match recv_result {
                    ...
                    Ok(Err(NodeConversationTermination::FiredAndForgotten)) => unimplemented!("FiredAndForgotten"),
    ...
    #[cfg(test)]
    pub fn tx_rx (&self) -> (Sender<OutgoingMessageType>, Receiver<Result<MessageBody, NodeConversationTermination>>) {
        (self.conversations_to_manager_tx.clone(), self.manager_to_conversation_rx.clone())
    }
}
...
mod tests {
    ...
    use masq_lib::messages::{UiShutdownResponse, UiShutdownRequest, UiUnmarshalError};
    use masq_lib::messages::ToMessageBody;
    use masq_lib::messages::FromMessageBody;
    use crossbeam_channel::TryRecvError;

    fn make_subject () -> (NodeConversation, Sender<Result<MessageBody, NodeConversationTermination>>, Receiver<OutgoingMessageType>) {
    ...
    fn transact_handles_successful_transaction () {
        ...
        assert_eq! (outgoing_message, OutgoingMessageType::ConversationMessage(UiShutdownRequest{}.tmb(42)));
    }

    #[test]
    #[should_panic(expected = "Cannot use NodeConversation::transact() to send message with MessagePath::FireAndForget. Use NodeCoversation::send() instead.")]
    fn transact_rejects_fire_and_forget_message () {
        let (subject, _, _) = make_subject();
        let message = UiUnmarshalError{ message: "Message".to_string(), bad_data: "Data".to_string() };
    ...
    fn transact_handles_resend_order() {
        let (subject, message_body_receive_tx, message_body_send_rx) = make_subject();
        message_body_receive_tx.send(Err(NodeConversationTermination::Resend)).unwrap();
        message_body_receive_tx.send(Ok (UiShutdownResponse{}.tmb(42))).unwrap();
    ...
    fn transact_handles_broken_connection() {
        let (subject, message_body_receive_tx, message_body_send_rx) = make_subject();
        message_body_receive_tx.send(Err(NodeConversationTermination::Fatal)).unwrap();
    ...
    fn send_handles_successful_transmission () {
        let (subject, message_body_receive_tx, message_body_send_rx) = make_subject();
        let message = UiUnmarshalError{ message: "Message".to_string(), bad_data: "Data".to_string() };
        message_body_receive_tx.send(Err (NodeConversationTermination::FiredAndForgotten)).unwrap();

        subject.send (message.clone().tmb(0)).unwrap();

        let (outgoing_message, context_id) = match message_body_send_rx.recv().unwrap() {
            OutgoingMessageType::FireAndForgetMessage(message_body, context_id) => (message_body, context_id),
            x => panic! ("Expected FireAndForgetMessage, got {:?}", x),
        };
        assert_eq! (UiUnmarshalError::fmb(outgoing_message).unwrap(), (message, 0));
        assert_eq! (context_id, subject.context_id());
    }

    #[test]
    #[should_panic(expected = "Cannot use NodeConversation::send() to send message with MessagePath::Conversation(_). Use NodeCoversation::transact() instead.")]
