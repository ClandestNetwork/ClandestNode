...
impl Command for ShutdownCommand {
    fn execute(&self, context: &mut dyn CommandContext) -> Result<(), CommandError> {
        ...
        let output: Result<UiShutdownResponse, CommandError> = transaction(input, context, STANDARD_COMMAND_TIMEOUT_MILLIS);
        match output {
            Ok(_) => (),
...
mod tests {
    ...
    fn shutdown_command_happy_path_immediate_receive() {
        ...
        assert_eq!(*transact_params, vec![(UiShutdownRequest {}.tmb(0), STANDARD_COMMAND_TIMEOUT_MILLIS)]);
        assert_eq!(
