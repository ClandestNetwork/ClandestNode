...
fn verify_bill_payment() {
    ...
    let consuming_node_connection = DbInitializerReal::new()
        .initialize(&consuming_node_path.clone().into(), cluster.chain_id, false)
        .unwrap();
    ...
    let serving_node_1_connection = DbInitializerReal::new()
        .initialize(&serving_node_1_path.clone().into(), cluster.chain_id, false)
        .unwrap();
    ...
    let serving_node_2_connection = DbInitializerReal::new()
        .initialize(&serving_node_2_path.clone().into(), cluster.chain_id, false)
        .unwrap();
    ...
    let serving_node_3_connection = DbInitializerReal::new()
        .initialize(&serving_node_3_path.clone().into(), cluster.chain_id, false)
        .unwrap();
...
fn expire_payables(path: PathBuf, chain_id: u8) {
    let conn = DbInitializerReal::new()
        .initialize(&path, chain_id, false)
        .unwrap();
